...
export class KinesisBridgeEnvelopeParser {
  ...
  public encode(envelope: KinesisBridgeEnvelope): Buffer {
    ...
    const sourcePropertiesString = JSON.stringify(envelope.sourceProperties);
    envelope.sourcePropertiesLength = Buffer.byteLength(sourcePropertiesString, 'utf8');
    const timeString = Buffer.from(uuid.parse(envelope.timeUUID)).toString('utf8');
    const targetPropertiesString = JSON.stringify(envelope.targetProperties);
    envelope.targetPropertiesLength = Buffer.byteLength(targetPropertiesString, 'utf8');
    ...
    writer.writeMultibyteNumLE(Buffer.byteLength(envelope.sourceName, 'utf8'), FIREHOSE_ENVELOPE.SOURCE_NAME_LENGTH);
    writer.writeString(envelope.sourceName, Buffer.byteLength(envelope.sourceName, 'utf8'));
    writer.writeMultibyteNumLE(envelope.sourcePropertiesLength, FIREHOSE_ENVELOPE.SOURCE_PROPERTIES_LENGTH);

    writer.writeString(sourcePropertiesString, envelope.sourcePropertiesLength);

    writer.writeMultibyteNumLE(Buffer.byteLength(envelope.targetName, 'utf8'), FIREHOSE_ENVELOPE.TARGET_NAME_LENGTH);

    writer.writeString(envelope.targetName, Buffer.byteLength(envelope.targetName, 'utf8'));

    writer.writeMultibyteNumLE(envelope.targetPropertiesLength, FIREHOSE_ENVELOPE.TARGET_PROPERTIES_LENGTH);
    writer.writeString(targetPropertiesString, envelope.targetPropertiesLength);

    writer.writeMultibyteNumLE(envelope.contentLength, FIREHOSE_ENVELOPE.CONTENT_LENGTH);
